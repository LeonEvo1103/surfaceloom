...
| Agent-callable service foundation | [`packages/service/src`](../../packages/service/src) | [`packages/service/tests`](../../packages/service/tests) | `contract-tested` | 70 项测试覆盖稳定 service 身份/catalog、本地不可变 Git snapshot、snapshot lease/隔离，以及 registered-argv command executor 的 allowlist、有界输出、deadline/cancel 和保守 cleanup。尚无 run/artifact store、MCP 或内建平台 process-tree controller；缺少 containment 证明时不得判绿。 |
