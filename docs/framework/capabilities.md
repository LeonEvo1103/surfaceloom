...
| Agent-callable service foundation | [`packages/service/src`](../../packages/service/src) | [`packages/service/tests`](../../packages/service/tests) | `contract-tested` | 101 项测试覆盖稳定 catalog、本地不可变 Git snapshot、registered-argv command executor、单进程 durable store、SurfaceLoom v3 executor，以及正式 MCP SDK v2 的 catalog/prepare/status/run/result/artifact/cancel。official client 已证明断线重连、`requestId` 恢复、真实 Node command 和 MCP→v3 kernel→Reporter bundle；业务失败、证据不足、传输/生命周期故障、协作取消和 cleanup unconfirmed 由结构化 `failureOrigin` 保持独立，未取得所有权的工作目录不会被清理。当前仅监听 loopback；一个 v3 service test 只映射一个 Case；prepare operation 不跨 service 重启持久化；没有远程 auth、多 writer store、数据库/分布式 scheduler 或内建平台 process-tree controller。 |
| LLM Judge + Reporter v3 integration | [`packages/llm-judge/src`](../../packages/llm-judge/src)、[`packages/test/src/judge`](../../packages/test/src/judge)、[`packages/reporter/src/v3/judge`](../../packages/reporter/src/v3/judge) | [`packages/llm-judge/tests`](../../packages/llm-judge/tests)、[`packages/test/tests/judge`](../../packages/test/tests/judge)、[`packages/reporter/tests/v3/judge`](../../packages/reporter/tests/v3/judge) | `contract-tested` | Case 可显式声明 Judge criterion；当前 attempt 的有界文本/PNG/JPEG/WebP evidence、Case deadline/cancel、三态结果和 correlation 进入 Reporter v3 required artifacts，retention off 也不能删除所需证据。确定性失败、证据不完整和 provider failure 均不能被改绿；尚无真实模型 provider 或 API smoke。 |
