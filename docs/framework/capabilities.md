...
| Agent-callable service foundation | [`packages/service/src`](../../packages/service/src) | [`packages/service/tests`](../../packages/service/tests) | `contract-tested` | 93 项测试覆盖稳定 catalog、本地不可变 Git snapshot、registered-argv command executor、单进程 durable store，以及正式 MCP SDK v2 的 catalog/prepare/status/run/result/artifact/cancel。official client 已证明断线重连、按 `requestId` 恢复、响应丢失不重复执行、显式取消、制品读取和 immutable Git snapshot 内的真实 Node command。当前仅监听 loopback；prepare operation 不跨 service 重启持久化；没有远程 auth、v3 service executor、多 writer store、数据库/分布式 scheduler 或内建平台 process-tree controller。 |
