...
| Agent-callable service foundation | [`packages/service/src`](../../packages/service/src) | [`packages/service/tests`](../../packages/service/tests) | `contract-tested` | 79 项测试覆盖稳定 service 身份/catalog、本地不可变 Git snapshot、registered-argv command executor，以及单机 `FileRunStore`/`FileArtifactStore` 的 dispatch 前持久化、`requestId` 幂等、重启隔离、有界取消和制品哈希校验。当前 store 明确为本地单 service 进程，不是数据库或分布式 scheduler；尚无 MCP、v3 service executor 或内建平台 process-tree controller。 |
