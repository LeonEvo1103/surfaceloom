...
| Agent-callable service foundation | [`packages/service/src`](../../packages/service/src) | [`packages/service/tests`](../../packages/service/tests) | `contract-tested` | 98 项测试覆盖稳定 catalog、本地不可变 Git snapshot、registered-argv command executor、单进程 durable store、SurfaceLoom v3 executor，以及正式 MCP SDK v2 的 catalog/prepare/status/run/result/artifact/cancel。official client 已证明断线重连、`requestId` 恢复、真实 Node command 和 MCP→v3 kernel→Reporter bundle；业务失败、协作取消和 cleanup unconfirmed 保持独立终态。当前仅监听 loopback；一个 v3 service test 只映射一个 Case；prepare operation 不跨 service 重启持久化；没有远程 auth、多 writer store、数据库/分布式 scheduler 或内建平台 process-tree controller。 |
