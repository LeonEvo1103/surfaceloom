...
| `SL-P5-060` | done | `SL-P5-040`,`SL-P3-094` | service SurfaceLoom v3 executor/tests | 通过公开 API 调用现有 v3 kernel，保留原 verdict、Reporter v3、deadline 与 cleanup；command 成功不能冒充其中所有 Case 通过 |
...
| `SL-P5-060` | `5664a85c483ae9be185d7b69afda10f7524e3c94` | `packages/service/src/executors/surfaceloom-v3/**`、service/MCP tests、`packages/test/src/runner-v3*`、release graph | `npm --prefix packages/service run typecheck && npm --prefix packages/service test && npm --prefix packages/test test && node --test scripts/tests/release-contract/*.test.mjs && ./scripts/check-architecture.sh && git diff --check` / 0 | macOS / Node 22 | 98 service tests + 365 test package tests + 37 release contracts / 0 skip | `packages/service/tests/executors/surfaceloom-v3`、`packages/service/tests/mcp/v3.test.ts` | 一个 service v3 定义精确映射一个已注册 Case；沿用公开 `runCaseV3`，不新增 runner。Reporter bundle、业务 verdict、deadline/cancel 与 cleanup receipt 已接通；尚无仓外 packed consumer、suite scheduler、Planner 或远程 auth。 |
...
| 2026-09-21 | 关闭 `SL-P5-060`：新增精确注册的单 Case v3 service executor，服务注入 `runId`、取消信号和私有工作目录，直接调用公开 `runCaseV3`；完整 Reporter v3 bundle 进入 ArtifactStore，CaseSpec correlation 经 MCP 返回。通过、业务失败、cleanup unconfirmed、协作取消和 official MCP client 纵向链均已覆盖；98 项 service tests、365 项 test package tests和 37 项 release contracts 通过。`SL-P5-070` 开放做仓外 consumer、断线与崩溃回归。 |
