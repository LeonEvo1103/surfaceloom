...
| `SL-P6-026` | review | `SL-P6-025` | llm-judge registered provider route、Codex CLI example/tests/docs | 数据配置只按 ID 引用可信代码注册的通用 `JudgeProvider`；不新增品牌核心类型、不让 llm-judge 依赖 service；Codex 示例验证结构化输出、图片、deadline、owned child exit 与临时目录清理，真实 Agent smoke 保持显式可选 |
