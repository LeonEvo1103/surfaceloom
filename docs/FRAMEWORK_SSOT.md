...
> 状态：Active
>
> 当前代码基线：`364e7d8aa442e246b14c615dfd23ee1ab26b3b46`
>
> 重新审计日期：2026-09-21
>
...
- service 已有 catalog、不可变本地 Git snapshot、registered-argv command executor、SurfaceLoom v3
...
| `SL-P3-095` | done | `SL-P1-051`,`SL-P3-085` | test 有界 observation/diagnostic 公开辅助 API 与行为测试 | 复用 deadline/observation；挂起读取有界返回，取消后不重读，迟到值不改写结果，保留 completeness 与停止未确认；诊断不覆盖原错误或充当 verdict |
| `SL-P3-096` | done | `SL-P3-091` | browser-playwright/v3 配置预设、session screenshot 预算与 consumer 验证 | 组合现有 backend、execution plan 和 runCaseV3 配置；policy 显式注入，默认不扩大授权，旧包根不强依赖 test；实际清理事实仍由 kernel 决定 |
| `SL-P3-097` | done | `SL-P3-095`,`SL-P3-096` | 通用辅助模块教程、两类消费者与真实 browser 验收 | 文档示例可执行；区分辅助诊断与必需证据；完整门禁通过，真实 browser preset 验证而非仅配置静态检查；不宣称 v3 自动截图或 target-app-tested |
...
| `SL-P5-040` | done | `SL-P5-020`,`SL-P5-030` | service run store/artifact store/tests | dispatch 前持久化 `runId` 及 snapshot/`testId`/参数/可选 `taskId`，按调用方 `requestId` 幂等；响应丢失重试返回原 run；断线可重读；重启后恢复 owned process 或隔离为 interrupted+tainted，未确认 cleanup 不释放 workspace；cancel 有界终态化 |
| `SL-P5-050` | done | `SL-P5-040` | service MCP server/SDK adapter/tests | 使用正式 MCP SDK 暴露 prepare/status/catalog/run/get-result/get-artifact/cancel；结构化 schema、四类 ID、`requestId` 与 lifecycle 同 service，不拼 shell、不复制 runner；断开 MCP 不等于取消 |
| `SL-P5-060` | done | `SL-P5-040`,`SL-P3-094` | service SurfaceLoom v3 executor/tests | 通过公开 API 调用现有 v3 kernel，保留 Reporter v3、deadline、cleanup 与结构化 failure origin；只有明确 criterion/合法非通过 Judge 标签是业务失败，insufficient 与基础设施故障不能冒充产品 verdict；command 成功不能冒充其中所有 Case 通过 |
| `SL-P5-070` | done | `SL-P5-050`,`SL-P5-060` | 中性 provider/consumer conformance | 独立 consumer 执行 Node/CLI 与 v3 任务，验证 prepare→run→reconnect→artifact→cancel，并注入“已启动但响应丢失”、service crash+存活 child、cleanup 永不确认；不得重复执行或复用 tainted workspace；私有消费证据只作补充 |
...
| `SL-P6-025` | done | `SL-P6-020` | llm-judge multi-profile router/tests | 同一环境 Key profile 可授权多个模型，同一模型可按显式 route 使用多个 profile；只对 retryable 429/5xx 有界切换，401、无效响应、证据不足、取消与 deadline 不切换；配置只保存环境变量名，返回非敏感路由尝试记录，并可适配现有 Case JudgeProvider 接口 |
| `SL-P6-026` | done | `SL-P6-025` | llm-judge registered provider route、Codex CLI example/tests/docs | 数据配置只按 ID 引用可信代码注册的通用 `JudgeProvider`；不新增品牌核心类型、不让 llm-judge 依赖 service；Codex 示例验证结构化输出、图片、deadline、owned direct-child exit 与临时目录清理，真实 Agent smoke 保持显式可选 |
...
| `SL-P6-060` | ready | `SL-P5-050`,`SL-P5-070` | `packages/service/src/planner/{contracts,tools}/**` 与 tests | Planner 只能 catalog/read/write patch/run/diff；外部 requirement/acceptance 为只读保护对象，CaseSpec 预期变化必须单独提出并经外部确认；patch 后冻结 revision并调用同一 service，返回 diff 与 `runId`；禁止 commit/push/merge |
...
`llm-judge → test → service` 构建拓扑。`SL-P4-040` 仍需补充可安装子集 profile，不能篡改旧快照或证据。consumer
...
| `SL-P6-025` | `2fb5a5ab2c9e78c531ae0dd5144586b532efee35` | `packages/llm-judge/src/router*.ts`、router tests/README、capability matrix | `npm --prefix packages/llm-judge test && npm --prefix packages/llm-judge run typecheck && npm --prefix packages/llm-judge pack --dry-run && ./scripts/run-typescript-tests.sh && ./scripts/run-swift-tests.sh && ./scripts/check-architecture.sh && node scripts/check-framework-ssot.mjs && git diff --check` / 0 | macOS / Node 22 / Chrome | 67 Judge + 全量 TypeScript 门禁 + 78 Swift / 0 required skip | `packages/llm-judge/tests/router.test.ts`、`packages/llm-judge/README.md` | 429/503 切换、401/无效响应不切换与多 Key/模型矩阵使用本地协议服务器验证；未使用真实多 Key API。路由尝试记录由直接 `classify` 返回，现有 Case provider 适配只返回最终 Judge outcome。 |
| `SL-P6-026` | `dce70349996d937a8a194804c6e4c1d4c460233b` | `packages/llm-judge/{src/router*,examples,tests,README.md}`、capability matrix | `npm --prefix packages/llm-judge test && npm --prefix packages/llm-judge run typecheck && npm --prefix packages/llm-judge pack --dry-run && ./scripts/run-typescript-tests.sh && ./scripts/run-swift-tests.sh && ./scripts/check-architecture.sh && node scripts/check-framework-ssot.mjs && git diff --check` / 0 | macOS / Node 22 / Chrome | 72 Judge + 全量 TypeScript 门禁 + 107 repository contracts + 78 Swift / 0 required skip | `packages/llm-judge/tests/router.test.ts`、`packages/llm-judge/tests/codex-exec-example.test.ts`、`packages/llm-judge/examples/codex-exec-provider.ts` | Codex binding 是可复制示例而非包内品牌 API；fake CLI 证明 direct child 与临时目录清理，不证明任意后代 process-tree containment；未调用真实 Codex 或消耗模型 token。 |
...
| `SL-P5-040` | `364e7d8aa442e246b14c615dfd23ee1ab26b3b46` | `packages/service/src/{persistent-service,run-lifecycle,stores}/**`、`packages/service/tests/{persistent-service,persistent-lifecycle,stores}/**` | `npm --prefix packages/service test && npm --prefix packages/service run typecheck && (cd packages/service && npm pack --dry-run) && ./scripts/run-framework-p1-tests.sh && node scripts/audit-publication.mjs` / 0 | macOS / Node 22 / Chrome | 17 persistence/lifecycle tests（93 service total）+ 7 required live browser Cases + 18 login fixture Cases / 0 required skip | `packages/service/tests/persistent-service.test.ts`、`packages/service/tests/persistent-lifecycle.test.ts`、`packages/service/tests/stores` | 单机单 service 进程；失败写入回滚索引，run store 超预算不破坏旧状态，同制品并发发布完整 bytes，cancel single-flight，workspace release 使用独立有界 signal。重启仍选择 interrupted+tainted 隔离；没有多进程 writer、数据库/对象存储或 v3 service executor。 |
| `SL-P5-050` | `364e7d8aa442e246b14c615dfd23ee1ab26b3b46` | `packages/service/src/agent-service.ts`、`packages/service/src/mcp/**`、`packages/service/src/parameter-normalization.ts`、`packages/service/tests/{agent-service,mcp}/**`、MCP SDK/NOTICE release evidence | `npm --prefix packages/service test && npm --prefix packages/service run typecheck && (cd packages/service && npm pack --dry-run) && node --test scripts/tests/release-contract/*.test.mjs && ./scripts/run-framework-p1-tests.sh && node scripts/audit-publication.mjs` / 0 | macOS / Node 22 / Chrome | 4 MCP/Agent service integration tests（93 service total）+ 37 release contracts + 7 required live browser Cases / 0 required skip | `packages/service/tests/mcp`、`packages/service/tests/agent-service.test.ts` | 正式 MCP SDK v2、loopback Streamable HTTP 与七个结构化工具已接通；official client 证明 reconnect、按 `requestId` 恢复、同请求只执行一次、artifact chunk、cancel 和 immutable Git snapshot 内真实 registered command。仅单 service 进程；prepare operation 不跨重启；无远程 auth、Planner 或 v3 service executor。 |
| `SL-P5-060` | `777fbb0a9bef55fc607c87ac9fabddbd324eb764` | `packages/service/src/executors/surfaceloom-v3/**`、service/MCP tests、`packages/test/src/{failure-origin,runner-v3*,judge/**}`、release graph | `npm --prefix packages/service run typecheck && npm --prefix packages/service test && npm --prefix packages/test test && node --test scripts/tests/release-contract/*.test.mjs && ./scripts/check-architecture.sh && git diff --check` / 0 | macOS / Node 22 | 101 service tests + 367 test package tests + 37 release contracts / 0 skip | `packages/service/tests/executors/surfaceloom-v3`、`packages/service/tests/mcp/v3.test.ts`、`packages/test/tests/failure-origin.test.ts` | 一个 service v3 定义精确映射一个已注册 Case；沿用公开 `runCaseV3`，不新增 runner。Reporter bundle、deadline/cancel、cleanup receipt 与结构化 failure origin 已接通；GPT-6 审计发现的未拥有目录误删和基础设施故障冒充业务失败已用反例关闭。尚无仓外 packed consumer、suite scheduler、Planner 或远程 auth。 |
| `SL-P5-070` | `f80632baef0955ba220c573fe6093c98e898524a` | `scripts/service-packed-consumer/**`、框架门禁接线 | `node scripts/service-packed-consumer/run.mjs && npm --prefix packages/service test && ./scripts/run-framework-p1-tests.sh && node scripts/audit-publication.mjs` / 0 | macOS / Node 22 / Chrome | 2 仓外 packed-consumer conformance + 101 service tests + 7 required live browser Cases + 18 login fixture Cases / 0 required skip | `scripts/service-packed-consumer/consumer`、`scripts/run-framework-p1-tests.sh` | 临时 consumer 位于仓库外，只加载安装后的 5 个候选 `.tgz`；覆盖 CLI/v3、正式 MCP client 重连/制品/取消、dispatch 后响应丢失、service crash+存活 child、cleanup unconfirmed、exactly-once 与 tainted workspace 拒绝复用。仍是候选包证据：当前必须一起安装内部闭包，`private`/`file:` 清理和 registry 安装属于 P4。生产 OS process-tree containment 仍由 embedding adapter 提供。 |
...
| `SL-P3-095` | `f7997036f1c16b911bfb21af4e8cae6a60825529` | `packages/test/src/runtime-helpers*`、对应 tests/README | `./scripts/run-typescript-tests.sh` + 独立早取消反例 / 0 | macOS 26.4.1 / Node 22.22.2 | test package 381/381，含 14 项 helper 回归 / 0 | `runtime-observation-helpers.test.ts`、`runtime-diagnostic-helper.test.ts` | 复用现有 deadline/observation；修复取消残留 poll timer 与伪造耗时。未确认 callback 停止仍为 unconfirmed，diagnostic 不发布制品、不充当 verdict。 |
| `SL-P3-096` | `f7997036f1c16b911bfb21af4e8cae6a60825529` | `packages/browser-playwright` preset、screenshot、packed consumers | `./scripts/run-typescript-tests.sh` + `SURFACELOOM_BROWSER_SMOKE=1 node node_modules/tsx/dist/cli.mjs --test --test-concurrency=1 tests/local-features.test.ts tests/local-smoke.test.ts tests/v3-live.test.ts` / 0 | macOS 26.4.1 / Chrome 153.0.8010.53 / owned headless | 66 browser contracts + 5 real browser tests，旧包根与 v3 packed consumer / 0 | `v3-run-options.test.ts`、`v3-live.test.ts`、`scripts/packed-consumer-v3.*` | 预设只组装现有内核，授权独立显式注入；截图正数预算下传 Playwright，零预算不提交。真实 Case 使用本地 fixture，owned cleanup 确认；无 v3 自动截图或产品验证声明。 |
| `SL-P3-097` | `f7997036f1c16b911bfb21af4e8cae6a60825529` | `docs/RUNTIME_HELPERS.md`、公开 file/HTTP recipe 与 tests | `npm --prefix examples/reference-agent test` + `npm --prefix examples/reference-agent run test:e2e` + `./scripts/run-swift-tests.sh` + architecture/SSOT/publication / 0 | macOS 26.4.1 / Node 22.22.2 / Chrome 153 | 49 example tests（含 4 recipe）+ 7 real E2E + 78 Swift / 0 | `examples/reference-agent/test/runtime-helpers.test.mjs`、`tests/e2e` | 主集成人全量复验：TypeScript 1022 pass/5 显式 live skip，另行串行 5/5 browser live；源代码公开审计无发现。两种只读来源共用 API，文件/服务真实清理；不接外部服务、不运行真实模型。 |
...
| 2026-09-21 | 关闭 `SL-P5-040`：按早期项目边界采用本地单进程 `runs.json` 与内容哈希制品目录，不引入数据库、迁移框架、分布式锁或对象存储抽象；run 在 dispatch 前持久化，`requestId` 重试复用原 run，重启把未终态运行隔离为 interrupted+tainted，有界 cancel 和 cleanup 门禁保证未确认时不释放 workspace。81 项 service tests、全框架 P1、7 个真实 Playwright Case 与公开扫描通过，开放 `SL-P5-050/060`。 |
...
| 2026-09-22 | `SL-P6-020` 审计窄修：OpenAI-compatible 仅接受顶层 `completed`，Anthropic 仅接受当前请求的 `end_turn`；截断、拒绝和失败响应即使残留合法 JSON 也返回 `invalidResponse`，不能形成分类。Judge README 同步为发布前真实 `.tgz` 安装路径；62 项 Judge、37 项 release、架构与 SSOT 门禁通过。 |
| 2026-09-22 | 关闭 `SL-P6-025`：新增通用多 profile/model Judge router；一个环境 Key 可声明多个允许模型，同一模型可由多把 Key 按 route 排序；只有 retryable 429/5xx 会切换候选，认证、无效响应、证据不足、取消和 deadline 保持 fail closed。配置不接收 Key 明文，67 项 Judge、全量 TypeScript、78 项 Swift、架构与 SSOT 门禁通过。 |
| 2026-09-22 | 关闭 `SL-P6-026`：Router 以独立可信注册表接入任意现有 `JudgeProvider`，数据配置只引用 ID；Codex CLI 保持仓库示例而非核心品牌类型。fake CLI 验证图片、结构化输出、deadline、direct child exit 与临时目录清理；72 项 Judge、全量 TypeScript、107 项 repository contracts、78 项 Swift、架构与 SSOT 门禁通过，未运行真实 Codex smoke。 |
| 2026-09-23 | 关闭 `SL-P3-095/096/097`：5.6 Sol 实现有界 observation/diagnostic、Playwright v3 薄预设及公开 file/HTTP recipe，主集成人独立审查并修正取消时残留 timer、虚构 elapsed 与截图零预算语义。全量 TypeScript、显式串行 Chrome、example/E2E、78 Swift 和架构/SSOT/公开审计通过；沿用现有 kernel 和第三方依赖，不增加空组件、不宣称 v3 自动截图或 target-app-tested。 |
