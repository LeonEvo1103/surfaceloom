...
| `SL-P5-050` | ready | `SL-P5-040` | service MCP server/SDK adapter/tests | 使用正式 MCP SDK 暴露 prepare/status/catalog/run/get-result/get-artifact/cancel；结构化 schema、四类 ID、`requestId` 与 lifecycle 同 service，不拼 shell、不复制 runner；断开 MCP 不等于取消 |
...
| `SL-P5-040` | `a26be182478be783245ee395080d5a04e9fe0d40` | `packages/service/src/{persistent-service,stores}/**`、`packages/service/tests/{persistent-service,stores}/**`、service/能力说明 | `npm --prefix packages/service test && npm --prefix packages/service run typecheck && (cd packages/service && npm pack --dry-run) && ./scripts/run-framework-p1-tests.sh && node scripts/audit-publication.mjs` / 0 | macOS / Node 22 / Chrome | 11 persistence/lifecycle tests（81 service total）+ 7 required live browser Cases + 18 login fixture Cases / 0 required skip | `packages/service/tests/persistent-service.test.ts`、`packages/service/tests/stores`、`scripts/run-framework-p1-tests.sh` | 单机单 service 进程、原子 `runs.json` 与本地内容哈希制品目录；重启时选择 interrupted+tainted 隔离而非接管存活进程。没有多进程 writer、数据库/对象存储、MCP 或 v3 service executor；参数不得承载凭据，保留策略由调用方负责。 |
