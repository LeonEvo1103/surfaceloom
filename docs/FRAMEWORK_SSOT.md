...
- service 已有 catalog、不可变本地 Git snapshot、registered-argv command executor、单进程 durable
  run/artifact store 与正式 MCP SDK v2 endpoint；已证明断线重连、`requestId` 恢复和显式 cancel。尚无
  v3 service executor、Planner adapter、远程 auth 或多进程 store；当前 CLI 的 v3 分支仍要求精确选择
  一个 Case，不是通用 suite runner。
...
| `SL-P5-060` | ready | `SL-P5-040`,`SL-P3-094` | service SurfaceLoom v3 executor/tests | 通过公开 API 调用现有 v3 kernel，保留原 verdict、Reporter v3、deadline 与 cleanup；command 成功不能冒充其中所有 Case 通过 |
