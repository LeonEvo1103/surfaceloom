...
| `SL-P5-070` | ready | `SL-P5-050`,`SL-P5-060` | 中性 provider/consumer conformance | 独立 consumer 执行 Node/CLI 与 v3 任务，验证 prepare→run→reconnect→artifact→cancel，并注入“已启动但响应丢失”、service crash+存活 child、cleanup 永不确认；不得重复执行或复用 tainted workspace；私有消费证据只作补充 |
