...
| Agent-callable service foundation | Contract-tested service identities/catalog, immutable local Git snapshots, a registered-argv command executor, and a small local persistent run/artifact store with request deduplication, restart recovery, bounded cancellation, and content-hash verification. MCP and the v3 service executor are not implemented yet |
...
Still missing are MCP, the SurfaceLoom v3 service executor, a real model
